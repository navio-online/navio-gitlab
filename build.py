...
def update_version(ver=None):
    with open('navio/meta_gitlab.py', 'r') as f:
        file_str = f.read()
    ...
    with open('navio/meta_gitlab.py', 'w') as f:
        f.write(file_str)

    nsh.git('commit', 'navio/meta_gitlab.py', '-m', 'Version updated to {}'.format(ver))
...
def create_tag():
    ...
    if m:
        ...
        raise "Can't find/parse current version in './navio/meta_gitlab.py'"
